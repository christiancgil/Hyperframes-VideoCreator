...
Cuando el usuario diga "edita este vídeo" o similar, ejecuta la Fase 0 (si aplica) y las 6 fases seguidas. Solo interrumpes si hay un error real que el usuario necesite resolver. **Cada fase tiene su validación — no pases a la siguiente sin completarla.**

---

### FASE 0 — Unión de clips fuente (solo si `/input/` tiene más de un vídeo)

Si el vídeo completo viene grabado en varios archivos (varias tomas, cámara que se paró, etc.), hay que unirlos en uno solo ANTES de transcribir o recortar nada. Si `/input/` solo tiene un vídeo, salta esta fase.

1. Detecta los clips en `/input/` con un único glob case-insensitive para no procesar el mismo archivo dos veces en filesystems case-insensitive (Windows/macOS):
   ```bash
   CLIPS=$(find input -maxdepth 1 -type f -iname "*.mp4" | sort)
   ```
   Si hay más de uno, son los clips fuente del mismo vídeo final.
2. **PRE-FLIGHT obligatorio en cada clip** — reutiliza las reglas de "Reglas críticas para multi-clip" (más abajo):
   ```bash
   for clip in $CLIPS; do
     echo "--- $clip ---"
     $FFPROBE_BIN -v quiet -select_streams v:0 \
       -show_entries stream=r_frame_rate,avg_frame_rate,width,height -of csv=p=0 "$clip"
     $FFMPEG_BIN -i "$clip" -vf freezedetect=n=0.001:d=0.5 -f null - 2>&1 | grep freeze_start
     $FFMPEG_BIN -i "$clip" -af silencedetect=n=-35dB:d=0.5 -f null - 2>&1 | grep -E "silence_start|silence_end"
   done
   ```
   Esto revela framerate real de cada clip (VFR → fuerza `-r 30` en el muxer al unir), freezes ya presentes en el material fuente (para no confundirlos luego con freezes introducidos por la unión) y silencios naturales en cada clip.
3. **Calcula y guarda el offset de cada clip en el vídeo unido** — esto es imprescindible para poder recortar en Fase 2 sin violar la regla de "nunca trimear desde el archivo concatenado":
   ```bash
   OFFSET=0
   echo "[" > input/clip_offsets.json
   FIRST=true
   for clip in $CLIPS; do
     DUR=$($FFPROBE_BIN -v quiet -show_entries format=duration -of csv=p=0 "$clip")
     [ "$FIRST" = true ] && FIRST=false || echo "," >> input/clip_offsets.json
     printf '  {"file": "%s", "start_in_merged": %s, "duration": %s}' "$clip" "$OFFSET" "$DUR" >> input/clip_offsets.json
     OFFSET=$(echo "$OFFSET + $DUR" | bc)
   done
   echo "" >> input/clip_offsets.json && echo "]" >> input/clip_offsets.json
   ```
4. Une los clips en orden con el demuxer concat. Si comparten codec/resolución/fps, stream copy en vídeo:
   ```bash
   for clip in $CLIPS; do echo "file '$PWD/$clip'"; done > input/concat_list.txt
   $FFMPEG_BIN -f concat -safe 0 -i input/concat_list.txt \
     -c:v copy -c:a aac -b:a 192k \
     input/video_completo.mp4 -y
   ```
   Si difieren en codec, resolución o fps, recodifica en su lugar (nunca `-c copy` en ese caso):
   ```bash
   $FFMPEG_BIN -f concat -safe 0 -i input/concat_list.txt \
     -c:v libx264 -preset slow -crf 18 -r $VIDEO_FPS \
     -c:a aac -b:a 192k \
     input/video_completo.mp4 -y
   ```
   **Nunca** el filtro `fps=fps=N` dentro de filter_complex para esto — solo `-r` en el muxer.
5. **Validación obligatoria de la unión:**
   ```bash
   # La duración del vídeo unido debe ser ≈ la suma de las duraciones de los clips fuente
   $FFPROBE_BIN -v quiet -show_entries format=duration -of csv=p=0 input/video_completo.mp4
   # Comprobar que no hay freezes NUEVOS introducidos justo en las fronteras entre clips
   $FFMPEG_BIN -i input/video_completo.mp4 -vf freezedetect=n=0.001:d=0.5 -f null - 2>&1 | grep freeze_start
   ```
6. Mueve los clips originales a `input/originales/` (no los borres) y actualiza las rutas de `input/clip_offsets.json` en consecuencia. `input/video_completo.mp4` es el único vídeo activo: la Fase 1 lo usa para transcribir, pero **la Fase 2 nunca corta directamente de él** — usa `clip_offsets.json` para traducir cada corte al clip fuente correspondiente (ver Fase 2).
...
2. Calcula los cortes óptimos **en el timeline del vídeo transcrito** (el de Fase 1 — `video_completo.mp4` si hubo Fase 0, o el único vídeo de `/input/` si no)
3. Ejecuta los cortes con **re-codificación de audio obligatoria** — nunca `-c copy` a secas:
   - **Si NO existe `input/clip_offsets.json`** (un solo vídeo fuente), corta directamente:
     ```bash
     $FFMPEG_BIN -i input.mp4 -ss 00:00:02 -to 00:00:15 \
       -c:v copy -c:a aac -b:a 192k \
       segmento_01.mp4 -y
     ```
   - **Si existe `input/clip_offsets.json`** (hubo Fase 0), **nunca cortes de `video_completo.mp4` directamente** — es un archivo concatenado y trimear ahí genera freezes o silencios artificiales en las fronteras (ver "Reglas críticas para multi-clip"). Traduce cada corte al clip fuente correspondiente:
     ```bash
     # merged_time = timestamp del corte en el timeline transcrito (video_completo.mp4)
     # busca en clip_offsets.json el clip cuyo start_in_merged <= merged_time < start_in_merged + duration
     # clip_time = merged_time - start_in_merged de ese clip
     $FFMPEG_BIN -i input/originales/CLIP_FUENTE.mp4 -ss CLIP_TIME_INICIO -to CLIP_TIME_FIN \
       -c:v copy -c:a aac -b:a 192k \
       segmento_01.mp4 -y
     ```
     Si un corte cruza la frontera entre dos clips, divídelo en dos cortes (uno por clip) y trátalos como segmentos consecutivos independientes.
...
- **Ejecuta sin interrumpir** — la única pausa obligatoria del pipeline es la Fase 4.5 (revisión visual en HyperFrames, esperar a que el usuario diga "renderiza"); la Fase 3 (plan de beats) se decide sin preguntar
...
### Reglas críticas para multi-clip (aprendidas en producción)

- **NUNCA trimear desde el archivo concatenado** — hacerlo desde clips fuente individuales. El merged/concat tiene PTSs discontinuas en las fronteras; trimear ahí genera freezes o silencios artificiales que no existen en los clips originales.
- **NUNCA usar el filtro `fps=fps=N` dentro de filter_complex** — duplica el último frame al encontrar gaps de PTS en VFR → freeze. Usar solo `-r 30` en el muxer (ajusta timestamps sin tocar frames).
- **NUNCA usar `aresample` con `first_pts=0`** en streams concatenados — inserta silencio artificial al inicio si el PTS del audio no es exactamente 0.
- **PRE-FLIGHT obligatorio antes de cualquier pipeline** con múltiples clips:

  ```bash
  # Para cada clip fuente ANTES de procesar:
  $FFPROBE_BIN -v quiet -select_streams v:0 \
    -show_entries stream=r_frame_rate,avg_frame_rate -of csv=p=0 clip.mp4
  $FFMPEG_BIN -i clip.mp4 -vf freezedetect=n=0.001:d=0.5 -f null - 2>&1 | grep freeze_start
  $FFMPEG_BIN -i clip.mp4 -af silencedetect=n=-35dB:d=0.5 -f null - 2>&1 | grep -E "silence_start|silence_end"
  ```

  Esto revela: VFR (→ `-r 30` obligatorio), freezes en fuente, y límites exactos de silencio por clip.
- **Calcular keep intervals en tiempo de cada clip fuente**, no en tiempo del merged:
  `clip_time = merged_time - clip_start_in_merged`
