...
**Regla de legibilidad (obligatoria):** cualquier segunda línea de texto bajo un titular (subtítulo, descripción secundaria) usa como mínimo el tamaño de "Texto de cards" de esta tabla — nunca el de "Labels y subtexto", que es solo para etiquetas muy cortas (badges, timestamps). Si un beat tiene titular + línea secundaria, la línea secundaria nunca baja de este mínimo.
