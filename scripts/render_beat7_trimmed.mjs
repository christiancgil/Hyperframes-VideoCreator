...
// beat_07 in new order = "Cada mirada, una experiencia" (was beat_06), trimmed to 3.5s
const htmlPath = path.join(ROOT, 'output', 'compositions', 'IMG_6460_voiceover', 'beat_06.html');
const frameDir = path.join(ROOT, 'output', 'verify', 'beat_frames', 'beat_7_trimmed');
const outputMov = path.join(ROOT, 'output', 'overlays', 'beat_7.mov');
...
const totalFrames = Math.ceil(3.5 * FPS); // 105 frames (trimmed from 6s to 3.5s)
console.log(`Rendering beat_07 (cada mirada, trimmed) — ${totalFrames} frames`);
