...
const AUDIO_PATH  = path.join(BASE, 'output', 'edl_restaurante_audio.mp3');
const OUTPUT_PATH = path.join(BASE, 'output', 'edl_restaurante_transcript.json');
...
const options = {
  ...
  headers: { 'Authorization': `Bearer ${API_KEY}`, 'Content-Type': `multipart/form-data; boundary=${boundary}`, 'Content-Length': body.length }
};
...
const req = https.request(options, res => {
  ...
  res.on('end', () => {
    ...
    console.log(`\nDuración total: ${json.duration?.toFixed(2)}s`);
    console.log('\nSegmentos:');
